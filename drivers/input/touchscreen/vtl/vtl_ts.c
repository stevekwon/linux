...
int vtl_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/*Probing TouchScreen*/
	printk("Probing vtl touchscreen, touchscreen node found\n");	
	...
	{
		err = devm_gpio_request(dev, ts->config_info.rst_gpio_number,NULL);
		if(err)
		{
...
/*Posible Powe Mangament Operations*/
